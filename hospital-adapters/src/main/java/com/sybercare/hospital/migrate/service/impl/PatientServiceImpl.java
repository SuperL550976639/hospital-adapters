		...
		if (users != null && !users.isEmpty()) {
			for (User user : users) {
				...
				if (userPatient != null) {
					...
					patient.setHospitalized_num(user.getCard_number());
					...
					SysPatientRelation sysPatientRelation = patientDao
							.selectSysPatientRelationByOldId(user.getUser_id());
					if (sysPatientRelation == null) {
						...
						patient.setPatient_id(sysPatientRelation.getNew_id());
